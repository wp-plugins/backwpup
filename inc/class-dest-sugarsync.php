...
// class methods
	...
	public function __construct( $email, $password ) {
		...
		if ( $curlgetinfo['http_code'] >= 200 && $curlgetinfo['http_code'] <= 204 ) {
	...
	private function doCall( $url, $data = '', $method = 'GET' ) {
		...
		// check auth token
		if ( ! is_string( $this->AuthToken ) || empty($this->AuthToken) || ! strripos( $this->AuthToken, self::API_URL ) )
