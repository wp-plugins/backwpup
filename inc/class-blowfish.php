...
class BackWPup_Blowfish {
   ...
   * @author Matt Harris
   **/
  public static function encrypt($plaintext, $key, $mode=BackWPup_Blowfish::BLOWFISH_MODE_CBC, $padding=BackWPup_Blowfish::BLOWFISH_PADDING_RFC, $iv=NULL) {
    if ( $mode == BackWPup_Blowfish::BLOWFISH_MODE_CBC and empty($iv) ) {
      throw new Exception('CBC Mode requires an IV key');
      return;
    }
    $ciphertext = '';
    $fish = new BackWPup_Blowfish($key, $mode, $padding, $iv);
    ...
    for ($i=0; $i < $len; $i+=$block) {
      if ($mode == BackWPup_Blowfish::BLOWFISH_MODE_CBC) {
   ...
   * @author Matt Harris
   **/
  public static function decrypt($ciphertext, $key, $mode=BackWPup_Blowfish::BLOWFISH_MODE_CBC, $padding=BackWPup_Blowfish::BLOWFISH_PADDING_RFC, $iv=NULL) {
    if ( $mode == BackWPup_Blowfish::BLOWFISH_MODE_CBC and empty($iv) ) {
      throw new Exception('CBC Mode requires an IV key');
      return;
    }
  ...
  private function _applyKey($key) {
    $this->P = BackWPup_Blowfish_DefaultKey::$P;
    $this->S = BackWPup_Blowfish_DefaultKey::$S;
  ...
  private function _pad($plaintext) {
    ...
    switch ($this->padding) {
      case BackWPup_Blowfish::BLOWFISH_PADDING_NONE:
        return $plaintext;
      case BackWPup_Blowfish::BLOWFISH_PADDING_RFC:
        $padder = $pad_len;
        break;
      case BackWPup_Blowfish::BLOWFISH_PADDING_ZERO:
        $padder = 0;
        break;
    }
 ...
 * @author Matt Harris
 **/
class BackWPup_Blowfish_DefaultKey {
