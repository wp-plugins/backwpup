...
function backwpup_update_option( $main, $name, $value ) {
	...
	if ( isset($alloptions[$main][$name]) || is_object( $oldvalue ) )
		$result = $wpdb->update( $wpdb->prefix . 'backwpup', array( 'value' => $value ), array( 'main' => $main,
																								'name' => $name ), array( '%s' ), array( '%s', '%s' ) );
	else
		$result = $wpdb->insert( $wpdb->prefix . 'backwpup', array( 'main'  => $main,
																	'name'  => $name,
																	'value' => $value ), '%s' );
	if ( $result ) {
