...
class BackWPup_Dest_Dropbox {
	...
	public function __construct( $boxtype = 'dropbox' ) {
		if ( $boxtype == 'dropbox' ) {
			$this->OAuthObject = new BackWPup_OAuthSimple(apply_filters( 'backwpup_api_appkey', 'DROPBOX_APP_KEY' ), apply_filters( 'backwpup_api_appkey', 'DROPBOX_APP_SECRET' ));
			$this->root        = 'dropbox';
		} else {
			$this->OAuthObject = new BackWPup_OAuthSimple(apply_filters( 'backwpup_api_appkey', 'DROPBOX_SANDBOX_APP_KEY' ), apply_filters( 'backwpup_api_appkey', 'DROPBOX_SANDBOX_APP_SECRET' ));
			$this->root        = 'sandbox';
		}
	}

	public function setOAuthTokens( $token, $secret ) {
		$this->OAuthToken = array( 'oauth_token' => $token,
								   'oauth_secret'=> $secret );
	}
	...
	public function upload( $file, $path = '', $overwrite = true ) {
		...
		if ( ! is_readable( $file ) || ! is_file( $file ) )
			throw new BackWPup_Dest_Dropbox_Exception("Error: File \"$file\" is not readable or doesn't exist.");
		if ( filesize( $file ) > 157286400 )
			throw new BackWPup_Dest_Dropbox_Exception("Error: File \"$file\" is too big max. 150 MB.");
	...
	public function metadata( $path = '', $listContents = true, $fileLimit = 10000 ) {
		...
		return $this->request( $url, array( 'list'	   => ($listContents) ? 'true' : 'false',
											'file_limit' => $fileLimit ) );
	}

	public function search( $path = '', $query, $fileLimit = 1000 ) {
		if ( strlen( $query ) >= 3 )
			throw new BackWPup_Dest_Dropbox_Exception("Error: Query \"$query\" must three characters long.");
		$url = self::API_URL . self::API_VERSION_URL . 'search/' . $this->root . '/' . $this->encode_path( $path );
		return $this->request( $url, array( 'query'	  => $query,
											'file_limit' => $fileLimit ) );
	}
	...
	public function fileopsDelete( $path ) {
		...
		return $this->request( $url, array( 'path' => '/' . $this->encode_path( $path ),
											'root' => $this->root ) );
	}
	...
	public function oAuthAuthorize( $callback_url ) {
		...
		if ( is_file( dirname( __FILE__ ) . '/Thawte_Premium_Server_CA.pem' ) )
			curl_setopt( $ch, CURLOPT_CAINFO, dirname( __FILE__ ) . '/Thawte_Premium_Server_CA.pem' );
		...
		if ( $status >= 200 && $status < 300 && 0 == curl_errno( $ch ) ) {
			...
			throw new BackWPup_Dest_Dropbox_Exception($message);
		}
		curl_close( $ch );
		$OAuthSign = $this->OAuthObject->sign( array(
		...
		return array( 'authurl'		   => $OAuthSign['signed_url'],
					  'oauth_token'	   => $oauth_token['oauth_token'],
	...
	public function oAuthAccessToken( $oauth_token, $oauth_token_secret ) {
		...
		$OAuthSign = $this->OAuthObject->sign( array(
			...
			'signatures'	  => array( 'oauth_token' => $oauth_token,
	...
	private function request( $url, $args = array(), $method = 'GET', $file = NULL, $echo = false ) {
		...
		if ( isset($output['error']) || $status['http_code'] >= 300 || $status['http_code'] < 200 || curl_errno( $ch ) > 0 ) {
...
class BackWPup_Dest_Dropbox_Exception extends Exception {
}
